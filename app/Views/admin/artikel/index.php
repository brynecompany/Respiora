...
<div class="header-card mb-4 d-flex align-items-center">
    ...
    <div>
        ...
        <small>Menampilkan list data artikel</small>
    </div>
            ...
            <table class="table align-middle">
                ...
                <?php foreach ($artikel as $a): ?>
                    <tr>
                        ...
                        <td><?= substr($a['deskripsi_artikel'],0,50); ?>...</td>
                        <td>
                            <?php if ($a['status_artikel'] == 'Publish'): ?>
                                <span class="badge bg-success">Publish</span>
                            <?php else: ?>
                                <span class="badge bg-secondary">Unpublish</span>
                            <?php endif; ?>
                        </td>
                        <td><?= date('d M Y', strtotime($a['tanggal_artikel'])); ?></td>
                        <td>
                            <a href="<?= base_url('admin/artikel/'.$a['id_artikel']) ?>"
                                class="btn btn-sm btn-info">
                                👁
                            </a>
                            <a href="<?= base_url('admin/artikel/edit/'.$a['id_artikel']) ?>"
                                class="btn btn-sm btn-warning">
                                <i class="bi bi-pencil"></i>
                            </a>
                            <button class="btn btn-sm btn-danger"
                                onclick="confirmDelete(<?= $a['id_artikel'] ?>)">
...
<div class="modal fade" id="filterModal" tabindex="-1">
  <div class="modal-dialog modal-dialog-centered">
    <div class="modal-content p-4">
      ...
      <h5 class="fw-bold mb-4">Filter Data Artikel</h5>
...
function confirmDelete(id) {
    const url = "<?= base_url('admin/artikel/delete/') ?>" + id;
